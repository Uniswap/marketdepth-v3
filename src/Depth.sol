...
contract Depth {
    ...
    function calculateMultipleDepth(address poolAddress, uint256[] calldata sqrtDepthX96, 
                                    bool[] calldata token0, bool[] calldata both) public returns (uint256[] memory) {
        ...
        for (uint i=0; i<sqrtDepthX96.length; i++) {
            tokenAmt = _calculateDepth(sqrtDepthX96[i], token0[i], both[i]);
    ...
    function calculateDepth(address poolAddress, uint256 sqrtDepthX96, bool token0, bool both) public returns (uint256) {
        ...
        return _calculateDepth(sqrtDepthX96, token0, both);
    }
    ...
    function _calculateDepth(uint256 sqrtDepthX96, bool token0, bool both) private returns (uint256) {
        uint256 returnAmt = 0;
        ...
        if (both) {
            returnAmt+=calculateOneSide(sqrtDepthX96, token0Ind);
            returnAmt+=calculateOneSide(sqrtDepthX96, !token0Ind);
        } else {
            returnAmt+=calculateOneSide(sqrtDepthX96, token0Ind);
        }
